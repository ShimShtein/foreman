...
FactoryGirl.define do
  ...
  factory :host do
    ...
    trait :with_config_group do
      config_groups { [ FactoryGirl.create(:config_group, :with_puppetclass, :class_environments => [puppet_aspect.try(:environment)]) ] }
    end
    ...
    trait :with_puppet_orchestration do
      managed
      with_puppet
      association :puppet_aspect, :factory => [:puppet_aspect, :with_orchestration]
      association :compute_resource, :factory => :libvirt_cr
      domain
      interfaces { [ FactoryGirl.build(:nic_primary_and_provision) ] }
    end
