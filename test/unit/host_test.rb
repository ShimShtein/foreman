...
class HostTest < ActiveSupport::TestCase
  ...
  test "sets compute attributes on create" do
    ...
    Host.create! :name => "myfullhost", :mac => "aabbecddeeff", :ip => "2.3.4.3",
      ...
      :puppet_aspect_attributes => { :puppet_proxy => smart_proxies(:puppetmaster),
  ...
  test "lookup value has right matcher for a host" do
    assert_difference('LookupValue.where(:lookup_key_id => lookup_keys(:five).id, :match => "fqdn=abc.mydomain.net").count') do
      Host.create! :name => "abc", :mac => "aabbecddeeff", :ip => "2.3.4.3",
        :domain => domains(:mydomain), :operatingsystem => operatingsystems(:redhat), :medium => media(:one),
        :subnet => subnets(:two), :architecture => architectures(:x86_64), :puppet_aspect_attributes => { :puppet_proxy => smart_proxies(:puppetmaster),
        :environment => environments(:production) }, :disk => "empty partition",
  ...
  context "location or organizations are not enabled" do
    ...
    test "should save if neither ptable or disk are defined when the host is not managed" do
      host = Host.create :name => "myfullhost", :mac => "aabbecddeeff", :ip => "2.3.4.03", :medium => media(:one),
        ...
        :subnet => subnets(:two), :architecture => architectures(:x86_64),
        :puppet_aspect_attributes => { :environment => environments(:production), :puppet_proxy => smart_proxies(:puppetmaster) },
        :managed => false
      assert host.valid?
    end
    ...
    test "should not save if IP is not in the right subnet" do
      if unattended?
        host = Host.create :name => "myfullhost", :mac => "aabbecddeeff", :ip => "123.05.02.03", :ptable => FactoryGirl.create(:ptable),
          ...
          :puppet_aspect_attributes => { :environment => Environment.first, :puppet_proxy => smart_proxies(:puppetmaster) }
        assert !host.valid?
      end
    end
    ...
    test "a fqdn Host should be assigned to a domain if such domain exists" do
      ...
      host = Host.create :name => "host.mydomain.net", :mac => "aabbccddeaff", :ip => "2.3.04.03",
        ...
        :architecture => architectures(:x86_64), :disk => "aaa",
        :puppet_aspect_attributes => { :environment => environments(:production) }
      host.valid?
    ...
    test "when changing host environment, its puppet classes should be verified" do
      h = FactoryGirl.create(:host, :puppet_aspect_attributes => { :environment => environments(:production) })
    ...
    test "when saving a host, do not require a puppet environment" do
      h = FactoryGirl.build(:host, :puppet_aspect_attributes => { :environment => environments(:production), :puppet_proxy => nil })
      h.puppet_aspect = nil
      assert h.valid?
    end

    test "when saving a host, require puppet environment if puppet master is set" do
      h = FactoryGirl.build(:host, :puppet_aspect_attributes => { :environment => environments(:production), :puppet_proxy => smart_proxies(:puppetmaster) })
      h.puppet_aspect.environment = nil
      refute h.valid?
    end
    ...
    test "should use a nested hostgroup parent root password" do
      ...
      g = h.hostgroup
      p = FactoryGirl.create(:hostgroup, :environment => h.puppet_aspect.environment)
    ...
    test "can search hosts by inherited puppet class from a hostgroup" do
      ...
      FactoryGirl.create(:host, :hostgroup => hg, :puppet_aspect_attributes => { :environment => hg.environment })
    ...
    test "can search hosts by puppet class from config group in parent hostgroup" do
      ...
      host = FactoryGirl.create(:host, :hostgroup => hostgroup, :puppet_aspect_attributes => { :environment => hostgroup.environment })
  ...
  test "parent_classes should return parent_classes if host has hostgroup and environment are the same" do
    ...
    host             = FactoryGirl.create(:host, 
                                          :hostgroup => hostgroup, 
                                          :puppet_aspect_attributes => { :environment => hostgroup.environment })
  ...
  test "parent_classes should not return parent classes that do not match environment" do
    ...
    host             = FactoryGirl.create(:host, :hostgroup => hostgroup, :puppet_aspect_attributes => { :environment => environments(:production) })
    assert host.hostgroup
    refute_empty host.parent_classes
  ...
  test "available_puppetclasses should return environment-specific classes" do
    ...
    assert_equal host.puppet_aspect.environment.puppetclasses.sort, host.available_puppetclasses.sort
  end
  ...
  test "available_puppetclasses should return environment-specific classes (and that are NOT already inherited by parent)" do
    ...
    host             = FactoryGirl.create(:host, :hostgroup => hostgroup, :puppet_aspect_attributes => { :environment => hostgroup.environment })
    refute_equal Puppetclass.scoped, host.available_puppetclasses
    refute_equal host.puppet_aspect.environment.puppetclasses.sort, host.available_puppetclasses.sort
    assert_equal (host.puppet_aspect.environment.puppetclasses - host.parent_classes).sort, host.available_puppetclasses.sort
  end

  test "#info ENC YAML omits root_pw when password_hash is set to Base64" do
    skip 'TODO: waiting for rebase'
    host = FactoryGirl.build(:host, :managed)
    host.hostgroup = nil
  ...
  test "#info ENC YAML uses all_puppetclasses for non-parameterized output" do
    ...
    host.puppet_aspect.expects(:all_puppetclasses).returns([myclass])
    enc = host.info
    assert_kind_of Hash, enc
  ...
  describe 'cloning' do
    ...
    test '#classes etc. on cloned host return the same' do
      ...
      pa = FactoryGirl.build(:puppet_aspect, :environment => hostgroup.environment)
      host = FactoryGirl.create(:host, :with_config_group, :with_puppetclass, :with_parameter, :puppet_aspect => pa, :hostgroup => hostgroup )
      copy = host.clone
  ...
  describe '#apply_inherited_attributes' do
    test 'should be no-op if no hostgroup selected' do
      host = FactoryGirl.build(:host, :managed)
      attributes = { 'puppet_aspect_attributes' => {'environment_id' => 1 } }
    ...
    test 'should take new hostgroup if hostgroup_id present' do
      ...
      assert_equal actual_attr['puppet_aspect_attributes']['environment_id'], new_environment.id
    end
    ...
    test 'should take old hostgroup if hostgroup not updated' do
      ...
      host = FactoryGirl.build(:host, :managed, :with_puppet, :with_hostgroup, :puppet_aspect_attributes => { :environment => environment})
      Hostgroup.expects(:find).never

      attributes = { 'hostgroup_id' => host.hostgroup.id }
      actual_attr = host.apply_inherited_attributes(attributes)

      assert_equal actual_attr['puppet_aspect_attributes']['environment_id'], host.hostgroup.environment.id
    end
    ...
    test 'should accept non-existing hostgroup' do
      ...
      assert_nil actual_attr['puppet_aspect_attributes']
    end

    test 'should not touch attribute set explicitly' do
      host = FactoryGirl.build(:host, :managed, :with_hostgroup)

      attributes = { 'hostgroup_id' => host.hostgroup.id, 'puppet_aspect_attributes' => { 'environment_id' => 1111 } }
      actual_attr = host.apply_inherited_attributes(attributes)

      assert_equal actual_attr['puppet_aspect_attributes']['environment_id'], 1111
    end
