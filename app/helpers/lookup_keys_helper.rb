module LookupKeysHelper
  ...
  def lookup_key_with_diagnostic(obj, lookup_key, lookup_value)
    ...
    parameter_value_content(
      ...
      :disabled => !lookup_key.overridden?(obj) || lookup_value.skip_foreman || !can_edit_params?,
  ...
  def skip_foreman_check_box(lookup_key, lookup_value, disabled)
    return unless lookup_key.type == "PuppetclassLookupKey"
    check_box(lookup_value_name_prefix(lookup_key.id), :skip_foreman,
              :value    => lookup_value.id,
              :disabled => disabled || !can_edit_params?,
              :onchange => "toggleSkipForemanValue(this, 'value')",
              :hidden   => disabled,
              :title    => _('Skip from ENC'),
              :checked  => lookup_value.skip_foreman)
  end
