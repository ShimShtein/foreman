class PuppetFacet < HostFacetBase
  ...
  def clear_puppetinfo(force = false)
    return unless host
    return if !force && environment && host.puppet_facet
  ...
  def self.inherited_attributes(hostgroup, facet_attributes)
    facet_attributes ||= {}
    inherited_attributes = hostgroup_inherited_attributes - facet_attributes.keys

    inherited_attributes.each do |attribute|
      value = hostgroup.send("inherited_#{attribute}")
      facet_attributes[attribute] = value
    end
    facet_attributes
  end

  def self.populate_fields_from_facts(host, importer, type, proxy_id)
    facet = host.puppet_facet || host.build_puppet_facet
    if Setting[:update_environment_from_facts]
      facet.set_non_empty_values importer, [:environment]
    else
      facet.environment ||= importer.environment unless importer.environment.blank?
    end

    facet.puppet_proxy_id ||= proxy_id
