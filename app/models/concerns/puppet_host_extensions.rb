module PuppetHostExtensions
  ...
  included do
    has_one :puppet_proxy, :class_name => 'SmartProxy', :through => :puppet_facet
    has_one :puppet_ca_proxy, :class_name => 'SmartProxy', :through => :puppet_facet
    ...
    alias_method :puppet_facet_attributes, :puppet_facet
    ...
    delegate :individual_puppetclasses, :classes_in_groups, :parent_classes, :classes, :to => :puppet_facet, :allow_nil => true

    include_in_clone :host_classes
  end
  ...
  def ensure_class_associations
    ...
    self.puppetclasses.select("puppetclasses.id,puppetclasses.name").uniq.each do |e|
      unless self.puppet_facet.environment.puppetclasses.map(&:id).include?(e.id)
        errors.add(:puppetclasses, _("%{e} does not belong to the %{environment} environment") % { :e => e, :environment => puppet_facet.environment })
        status = false
      end
    end if self.puppet_facet.try(:environment)
  ...
  def set_hostgroup_puppet_defaults
    self.puppet_facet.set_hostgroup_defaults if puppet_facet
  end

  def available_puppetclasses
    return puppet_facet.available_puppetclasses if puppet_facet

    Puppetclass.scoped
  end

  def puppet_facet=(val)
    puppet_facet.clear_puppetinfo(true) if val.nil? && puppet_facet
    super
  end

  # Determine if host is setup for configuration
  def configuration?
    puppet_facet && puppet_facet.puppet_proxy_id.present?
  end
end
