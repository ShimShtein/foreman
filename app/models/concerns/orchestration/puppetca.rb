module Orchestration::Puppetca
  ...
  def initialize_puppetca
    return unless puppet_facet && puppet_facet.puppetca?
    return unless Setting[:manage_puppetca]
    @puppetca = ProxyAPI::Puppetca.new :url => puppet_facet.puppet_ca_proxy.url
    true
  rescue => e
  ...
  def queue_puppetca
    return unless puppet_facet && puppet_facet.puppetca? and errors.empty?
    return unless Setting[:manage_puppetca]
