...
function toggleOverrideValue(item) {
  ...
  var fields_to_disable = fields.find("[name$='[required]'],[id$='_key_type'],[id$='_validator_type'],[name$='[skip_foreman]'],[name$='[hidden_value]']");
  var skip_foreman = $(item).closest('fieldset').find("[id$='skip_foreman']").is(':checked');
  ...
  default_value_field.prop('disabled', !override || skip_foreman);
...
function toggleSkipForemanValue(item, value_field) {
  var skip_foreman = $(item).is(':checked');
  $(item).closest('.fields').find('[id$=' + value_field + ']').prop('disabled', skip_foreman);
}
